...
class FundUsageClassificationTool:
    """根据资金用途提示获取分类信息工具类"""
    
    def __init__(self, host: str = "10.1.93.208", port: int = 8090):
        self.url = f"http://{get_config("edss_api.host") or host}:{get_config("edss_api.port") or port}/services/403502"
        self.headers = {'Content-Type': 'application/xml'}

    def execute(self, funds_tip_id: str, standard_type: str) -> Dict:
        """执行资金用途分类查询
        
        Args:
            funds_tip_id: 资金用途提示ID
        ...
        <!-- "Service URL": "http://[host:port]/services/403502", "Service Name": "根据资金用途提示获取分类信息", "Transaction Code": "GFM0003", "Transaction Type": "null" -->
        <reqt>
            <svcHdr>                                                                    <!-- -->
                <corrId></corrId>                                                       <!-- Correlation Id 可选项 元素类型:A 长度: 精度:  备注:服务关联Id-->
                <svcId>403502</svcId>                                                   <!-- Service Id 可选项 元素类型:A 长度: 精度:  备注:服务Id-->
            ...
            <appHdr>
                <bsnCde>403502</bsnCde>
                <chl>AIJ</chl>
                <reqDt>20250715</reqDt>
                <reqTm>182316</reqTm>
            </appHdr>
            <appBody>
                <fundsTipId>{funds_tip_id}</fundsTipId>
