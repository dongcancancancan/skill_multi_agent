...
### 五、标准化行动与结构化输出机制（基于MetaGPT架构）
针对当前单智能体偶尔出现的幻觉、指令遵循能力弱以及输出格式不规范等问题，引入MetaGPT框架中的“强工程约束”思想，通过代码封装和结构化提示技术，将自然语言的不确定性转化为工程实现的确定性。

1. 核心机制一：行动的类对象化（Action as a Class）
理论依据： 在常规的LangChain开发中，Tool或Action往往仅定义为描述性的字符串提示。MetaGPT提出了将Action定义为**Python类（Class）**的范式。通过类的封装，将一个“模糊的意图”转化为一个具有明确生命周期、前置依赖检查和重试逻辑的“原子化程序对象”。

技术对比：

传统模式：基于文本的Prompt指令（如 "请分析该企业的财务风险..."），依赖LLM的自觉性，缺乏运行时约束。

MetaGPT模式：强制实例化Action类，代码即约束。
...
# 传统写法 (当前项目可能存在的隐患)：
# 仅依赖Prompt，无法确保上下文依赖是否满足
prompt = "请基于上传的财报分析该企业的绿色信贷资质"

# 优化后的类对象化写法 (参考MetaGPT)：
class AnalyzeGreenCredit(Action):
    def __init__(self, context: UniState):
        # 1. 前置依赖强校验：代码层面锁死执行条件
        # 如果上下文中没有财报数据，直接抛出异常，根本不进LLM，节省Token
        if not context.get('financial_report'):
             raise PreconditionError("缺少财报数据，无法执行信贷分析")
        self.context = context

    async def run(self):
        # 2. 内置重试与自我修正逻辑
        try:
            result = await self._aask(prompt=PROMPT_TEMPLATE)
...
消除不确定性：将Agent从“自由聊天”转变为“执行函数”，确保每次调用的入参和环境都经过代码级校验。

鲁棒性提升：通过类内部的异常捕获和重试机制，大幅提升单一节点的成功率。

2. 核心机制二：结构化提示工程（Structured Prompting）
理论依据： 为了解决金融分析中输出内容发散、关键指标遗漏的问题，借鉴MetaGPT的**“填空题化”**提示策略。不给LLM“自由发挥”的空间，而是提供极度精细的Schema模板，强制LLM进行结构化填充。

实施策略： 将开放式问答转化为Markdown/JSON Schema填空。这不仅规范了输出，更利用了**约束即CoT（思维链）**的原理——强迫LLM按照模板定义的逻辑顺序（先目标、再分析、后结论）进行推理。
...
# 角色定义
你是一名资深的蓝绿金融审核专家，请基于提供的企业数据，完成《绿色金融准入预审报告》。

# 任务约束
必须严格遵守以下Markdown结构进行输出，不要包含任何客套话：

## 1. 准入判定 (Access Determination)
(请明确输出：通过/拒绝/需补充材料，并给出一个置信度评分 0-100)

## 2. 关键指标核查 (Key Metrics Check)
| 指标项 | 提取数值 | 阈值要求 | 是否达标 |
| :--- | :--- | :--- | :--- |
| 绿色收入占比 | (填入%) | >30% | (是/否) |
| 环保处罚次数 | (填入次) | =0 | (是/否) |

## 3. 风险因素分析 (Risk Analysis)
(请列出不少于3点潜在风险，按严重程度P0-P2排序)

## 4. 政策匹配度 (Policy Matching)
(列出该企业符合的绿色金融政策条款ID及匹配理由)
优化价值：

消除歧义：LLM不需要猜测报告包含什么章节，只需像填Excel一样填充内容。

精准度提升：通过“表格化”和“分段式”Prompt，强制开启思维链推理，显著减少计算错误和逻辑跳跃。

下游集成便利：结构化的输出使得后端可以轻松通过正则或解析器提取核心数据（如“绿色收入占比”），便于存入数据库或生成前端图表。


### 六、上下文工程与LangChain文档设计优化
